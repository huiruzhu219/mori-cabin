...
export default function MoodChart({ entries }: MoodChartProps) {
  ...
  return (
    <div className="relative h-[86px] rounded-2xl border border-dashed border-[#eadfce] bg-[#fffdf8] overflow-hidden">
      ...
      {points.map(({ entry, x, y }) => (
        ...
        >
          <span className="text-[24px] drop-shadow-sm">{entry.mood}</span>
          <span className="mt-0.5 text-[10px] font-bold text-[#a0907d]">{formatDate(entry.date)}</span>
        </div>
      ))}
    </div>
  );
