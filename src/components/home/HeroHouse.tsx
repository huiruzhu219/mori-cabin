...
export default function HeroHouse({ onOpenSummary }: HeroHouseProps) {
  ...
  return (
    <button
      ...
      className="relative w-full rounded-[28px] bg-white border border-[#e7dccb] p-4 shadow-[0_6px_18px_rgba(93,84,73,0.06)] text-left"
    >
      <div className="washi-tape-green -top-2 left-8 h-5 w-[110px] opacity-50" />
      <div className="relative aspect-[1.46] rounded-[22px] overflow-hidden bg-[#f3f2ef] shadow-inner">
        {imageFailed ? (
        ...
        <div className="absolute left-5 bottom-4 rounded-full bg-white/95 px-4 py-2 shadow-[0_3px_10px_rgba(93,84,73,0.14)] flex items-center gap-2">
          <span className="w-3.5 h-3.5 rounded-full bg-[#f0c7b6]" />
          <span className="text-base font-bold text-[#7a6b4c] max-[520px]:text-xs">小屋暖暖的，风里有花香 🌸</span>
        </div>
      </div>
    </button>
  );
